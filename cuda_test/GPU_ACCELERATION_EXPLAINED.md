...
| Variant | Launch pattern | Overhead |
|---|---|---|
| **Serial** (`bloom_filter_kernel`) | `<<<1, T>>>` launched once per data block in a loop, with a `cudaDeviceSynchronize` + H2D/D2H transfer between each block | High — ~1000 round trips for a typical dataset |
| **Batched** (`bloom_filter_kernel_batched`) | `<<<num_blocks, T>>>` launched once for ALL data blocks. 1× H2D transfer, 1 kernel launch, 1× D2H transfer | **Low** — eliminates all sync overhead |
...
| **2 — Bloom** | CPU bloom vs GPU bloom (kernel-only, serial per-block, and batched) |
| **3 — Total Compaction** | Combined compaction time: 7 different CPU/GPU path combinations. If `--fillrandom_keys` is provided (e.g. `10M`, `200M`), also projects single-round timings across N compaction rounds to simulate a `db_bench fillrandom` workload |
...
3. **Transfer overhead** is small relative to compute savings; the batched approach eliminates the per-block `cudaDeviceSynchronize` that kills the serial GPU path
