...
# Lives in benchmarks/common/. Sources machine-local config from benchmarks/common/config/.env.local.

COMMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BENCH_ROOT_DIR="$(cd "${COMMON_DIR}/.." && pwd)"
BENCH_ENV_FILE="${COMMON_DIR}/config/.env.local"
...
CONFIG_KEYS=(
  ...
  WAL_BASE_DIR
  OUTPUT_DIR
  CPU_COMPACTION_OUT_ROOT
  CPU_COMPACTION_PRELOAD_ROOT
  GPU_COMPACTION_OUT_ROOT
  GPU_COMPACTION_DATASET_ROOT
...
# Metadata keys for run_config.env — derived from CONFIG_KEYS to avoid duplication.
RUN_METADATA_KEYS=("${CONFIG_KEYS[@]}")
...
start_host_metrics_collection() {
  ...
  local collector_script="${COMMON_DIR}/collect_host_metrics.py"
  local device
