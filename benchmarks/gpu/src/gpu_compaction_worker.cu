/*
 * gpu_compaction_worker.cu  —  Real GPU compaction worker.
 *
 * Reads N input SST files via RocksDB SstFileReader, runs the GPComp
 * merge kernel on the GPU, then writes the merged result as a single
 * output SST using SstFileWriter.
 *
 * Usage:
 ...
 *       [--key_digits  N]   (decimal digit count, default 6)
 ...
 *
 * Key parsing:
 *   Keys are formatted as "<prefix><zero-padded integer>" (e.g. "key000042").
 *   --key_prefix sets the number of prefix bytes to skip.
 *   The integer part is parsed with strtoull(); it becomes the uint64_t sort
 *   key for the GPU merge.  For same user-key appearing in multiple input
 *   SSTs, the entry from the LAST-listed (newest) SST is kept in the output.
 *
 * Design notes:
 *   - Assumes non-duplicate user keys WITHIN a single SST (NewIterator())
 *   - Handles same user key in multiple SSTs by taking the newest version
 *   - Bloom filters are built automatically by SstFileWriter
 *   - cuFile / GDS is NOT used here (cudaMemcpy for H2D/D2H)
 */
...
/* ---- include existing GPU kernels (header-only, nvcc required) ---------- */
 ...
 *
 * We parse the numeric suffix of the key (after skipping `key_prefix` bytes)
 * and shift it left by KEY_SORT_SHIFT bits, then OR in a "sst age" value so
 * that if the same user key appears in multiple SSTs the NEWEST one (highest
 * sst_index) sorts FIRST — enabling a simple "keep-first" dedup pass.
 *
 * Layout (64 bits):
...
int main(int argc, char** argv) {
    ...
    const double t_h2d_start = now_us();

    /* Build raw-pointer arrays expected by launch_merge */
    ...
    const double t_h2d_end = now_us();

    /* launch_merge handles all D-memory allocation, H2D, kernel, D2H */
    const double t_kernel_start = now_us();
    int rc = launch_merge(h_sst_ptrs.data(), h_sst_sizes.data(),
                          num_ssts, h_output.data());
    const double t_kernel_end = now_us();

    if (rc != 0) {
        fprintf(stderr, "launch_merge failed (rc=%d)\n", rc);
        return EXIT_FAILURE;
    }

    /* launch_merge includes H2D and D2H internally; we track total time
     * excluding the staging time measured above. */
    const double h2d_us        = t_h2d_end - t_h2d_start;
    const double merge_kernel_us = t_kernel_end - t_kernel_start;

    /* ---- Step 3: CPU dedup — resolve same-user-key across SSTs -------- */
    /*
     * After merge the output is sorted by sort_key, which encodes
     * (user_key_int, sst_age).  For the same user_key_int, entries from the
     * NEWEST SST sort FIRST (smallest age bits).  We keep the first
     * occurrence of each user_key_int and discard the rest.
     */
    ...
    for (const KVPair& kv : h_output) {
        ...
        /* else: duplicate user_key from an older SST — discard */
    }
    ...
    /* D2H is included in merge_kernel_us (launch_merge is synchronous) */
