...
# Metrics-specific defaults (override common GPU_ONLY default)
...
SUMMARY_BIN="${BENCH_DIR}/plotting/summarize_gpu_metrics.py"
...
for VAL in ${VALUES_STR}; do
    ...
    for MODE in ${MODES_STR}; do
        ...
        if [[ "${COLLECT_NSYS}" == "1" ]]; then
            ...
            gpu_sweep_bench_args "${DATASET_DIR}" "${NSYS_BENCH_OUTDIR}" "${NSYS_PROFILE_RUNS}" "${MODE}"

            if run_nsys_profile "${NSYS_PREFIX}" "${NSYS_LOG_PATH}" "${BENCH_ARGS[@]}"; then
                NSYS_EXIT_CODE="0"
            else
                NSYS_EXIT_CODE="$?"
        ...
        if [[ "${COLLECT_NCU}" == "1" ]]; then
            ...
            gpu_sweep_bench_args "${DATASET_DIR}" "${NCU_BENCH_OUTDIR}" "${NCU_PROFILE_RUNS}" "${MODE}"

            if run_ncu_profile "${NCU_PREFIX}" "${NCU_LOG_PATH}" "${BENCH_ARGS[@]}"; then
                NCU_EXIT_CODE="0"
            else
                NCU_EXIT_CODE="$?"
