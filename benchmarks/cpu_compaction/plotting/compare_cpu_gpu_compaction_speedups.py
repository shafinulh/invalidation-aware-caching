...
def load_cpu_rows(
    ...
    baseline_df = case_df[case_df["requested_subcompactions"] == 1][
        ["value_size", "total_compaction_wall_s", "total_compaction_wall_s_std"]
    ].copy()
    baseline_df["rocksdb_wall_ms"] = pd.to_numeric(
        baseline_df["total_compaction_wall_s"], errors="coerce"
    ) * 1000.0
    baseline_df["rocksdb_wall_std_ms"] = pd.to_numeric(
        baseline_df["total_compaction_wall_s_std"], errors="coerce"
    ).fillna(0.0) * 1000.0
    baseline_df = baseline_df[["value_size", "rocksdb_wall_ms", "rocksdb_wall_std_ms"]]

    return pd.DataFrame(plot_rows), best_cpu_df, baseline_df
        ...
        rows.append(
            {
                ...
                "cpu_mean_ms": float(parsed["cpu_mean_ms"]),
                "cpu_std_ms": float(parsed["cpu_std_ms"]),
                "gpu_mean_ms": float(parsed["gpu_mean_ms"]),
                "gpu_std_ms": float(parsed["gpu_std_ms"]),
...
def build_rocksdb_normalized_gpu_df(
    gpu_df: pd.DataFrame,
    rocksdb_baseline_df: pd.DataFrame,
) -> pd.DataFrame:
    baseline_by_value = {
        int(row.value_size): {
            "rocksdb_wall_ms": float(row.rocksdb_wall_ms),
            "rocksdb_wall_std_ms": float(row.rocksdb_wall_std_ms),
        }
        for row in rocksdb_baseline_df.itertuples(index=False)
    }

    rows: list[dict[str, object]] = []
    for row in gpu_df.itertuples(index=False):
        value_size = int(row.value_size)
        baseline = baseline_by_value.get(value_size)
        if baseline is None:
            raise SystemExit(
                f"missing RocksDB subcomp=1 baseline for GPU value_size={value_size}"
            )

        rocksdb_ms = baseline["rocksdb_wall_ms"]
        rocksdb_std_ms = baseline["rocksdb_wall_std_ms"]
        gpu_ms = float(row.gpu_mean_ms)
        gpu_std_ms = float(row.gpu_std_ms)
        speedup = rocksdb_ms / gpu_ms if gpu_ms > 0 else 0.0
        speedup_std = 0.0
        if rocksdb_ms > 0 and gpu_ms > 0:
            speedup_std = speedup * math.sqrt(
                (rocksdb_std_ms / rocksdb_ms) ** 2 + (gpu_std_ms / gpu_ms) ** 2
            )

        rows.append(
            {
                "sst_size_mb": int(row.sst_size_mb),
                "input_sst_count": int(row.input_sst_count),
                ...
                "speedup": speedup,
                "speedup_std": speedup_std,
                "speedup_min": float(getattr(row, "speedup_min", 0.0) or 0.0),
                "details": (
                    "GPComp speedup computed from RocksDB subcomp=1 wall time over GPU wall time; "
                    "std uses propagated RocksDB/GPU timing stddev"
                ),
...
def main() -> int:
    ...
    cpu_plot_df, best_cpu_df, rocksdb_baseline_df = load_cpu_rows(
    ...
    gpu_rocksdb_df = build_rocksdb_normalized_gpu_df(gpu_df, rocksdb_baseline_df)
    rocksdb_combined_df = pd.concat([cpu_plot_df, gpu_rocksdb_df], ignore_index=True).sort_values(
        ["value_size", "method_group", "method_label"]
    )
    ...
    rocksdb_png_path = plot_comparison(
        rocksdb_combined_df,
        args.output_dir,
        args.sst_size_mb,
        args.input_sst_count,
        ...
        gpu_modes,
        "rocksdb_speedup",
        "Speedup vs RocksDB subcomp=1",
        "All bars are normalized to RocksDB subcomp=1. GPU bars show RocksDB single-threaded compaction wall time over GPU wall time.",
    )
    ...
    print(f"Wrote {rocksdb_png_path}")
    return 0
