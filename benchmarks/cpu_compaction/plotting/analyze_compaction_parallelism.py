...
def parse_run(run_dir: Path) -> tuple[dict[str, object], dict[str, pd.DataFrame]]:
    metadata_path = run_dir / "metadata" / "cpu_compaction_sweep.env"
...
def main() -> int:
    ...
    run_dirs = sorted(
        ...
        for metadata_path in experiment_root.rglob("cpu_compaction_sweep.env")
    )
    if not run_dirs:
        raise SystemExit(f"no cpu_compaction_sweep runs found under {experiment_root}")
