...
static inline GPUCompactionResult gpu_q_compaction_paper_overlap_from_parsed(const std::vector<ParsedSST>& inputs)
{
    GPUCompactionResult result;
    std::vector<GPUUnpackStreamState> unpack_states(inputs.size());
    ...
    t0 = std::chrono::steady_clock::now();
    GroupPrecompressState precompress = launch_group_precompress_async(merged.d_output, merged.total);
    ...
    t0 = std::chrono::steady_clock::now();
    cudaStreamSynchronize(precompress.stream);
    float precompress_kernel_ms = 0.0f;
    cudaEventElapsedTime(&precompress_kernel_ms, precompress.kernel_start, precompress.kernel_stop);
    PackTimedResult pack = launch_pack_timed_from_precompressed_groups_plan_spans(
        precompress.d_group_payloads, precompress.d_group_sizes,
        device_plans.d_first_kv, device_plans.d_num_kv, plans, pack_spans);
    std::vector<Key128> largest_keys =
    ...
    t1 = std::chrono::steady_clock::now();
    result.stage.pack_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
    result.pack_kernel_ms = std::max(pack.kernel_ms, precompress_kernel_ms);

    destroy_group_precompress_state(precompress);
