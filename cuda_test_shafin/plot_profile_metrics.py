...
def plot_32_vs_1024_analysis(csv_path, graph_dir):
    import csv
    if not os.path.exists(csv_path):
        print(f"Summary CSV not found at {csv_path}, skipping 32v1024 plot.")
        return
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            if not row['value_bytes'].isdigit(): continue
            val = int(row['value_bytes'])
            if val not in [32, 1024]: continue
            mode = row['mode']
            if mode not in data:
                data[mode] = {}
            data[mode][val] = row
    ...
    vals = [32, 1024]
    if not all(m in data for m in modes):
        return

    # Plot 1: Nsight Profile: Overall Time
    fig, ax = plt.subplots(figsize=(10, 6))
    x = np.arange(len(vals))
    width = 0.15
    cpu_times = [float(data['q_paper_with_plan'][v]['cpu_wall_ms']) for v in vals]
    ax.bar(x - 2.5*width, cpu_times, width, label='CPU Baseline', color='black')
    
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']
    for i, mode in enumerate(modes):
        gpu_times = [float(data[mode][v]['gpu_wall_ms']) for v in vals]
        ax.bar(x + (i - 1.5)*width, gpu_times, width, label=f'GPU {mode}', color=colors[i])

    ax.set_ylabel('Execution Time (ms)')
    ax.set_title('Nsight Profile: Overall Time (CPU vs GPU variants) for 32B and 1024B')
    ax.set_xticks(x)
    ax.set_xticklabels([f'{v}B' for v in vals])
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(graph_dir, 'nsight_profile_overall_time_comparison.png'))
    plt.close()

    # Plot 2: GPU Breakdown
    components = [
        ('Read+Parse', 'gpu_read_parse_ms'),
        ('Unpack', 'gpu_unpack_ms'),
        ('Sort+Merge', 'gpu_sort_merge_ms'),
        ('Plan', 'gpu_plan_ms'),
        ('Pack+Assemble', 'gpu_pack_assemble_ms'),
        ('Write', 'gpu_write_ms'),
        ('H2D Transfer', 'h2d_ms'),
        ('D2H Transfer', 'd2h_ms'),
    ]
    ...
    for vi, val in enumerate(vals):
        ...
        for comp_name, comp_key in components:
            values = [float(data[mode][val][comp_key]) for mode in modes]
            ax.bar(modes, values, bottom=bottoms, label=comp_name)
            bottoms += np.array(values)
        ax.set_title(f'GPU Breakdown for {val}B Values')
        ax.set_xticklabels(modes, rotation=30, ha='right')
        if vi == 0: ax.set_ylabel('Time (ms)')

    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper center', ncol=4)
    plt.tight_layout(rect=[0, 0, 1, 0.9])
    plt.savefig(os.path.join(graph_dir, 'nsight_profile_gpu_time_breakdown.png'))
    plt.close()
    print("Saved 32v1024 comparison graphs.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv_summary', default='sweep_results/sweep_nsight_nsys_8mb-sst_24sst/nsight_summary_metrics.csv',
                        help='Path to summary ms CSV')
    args = parser.parse_args()
    
    plot_latest_sweep_profile_metrics(args.results_dir, args.graphs_dir)
    plot_32_vs_1024_analysis(args.csv_summary, args.graphs_dir)
