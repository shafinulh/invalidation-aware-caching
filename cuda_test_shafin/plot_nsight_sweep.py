...
def parse_bench_log(log_path):
    ...
    cpu_wall = re.search(r'CPU total \(Wall\):\s*min=([\d.]+)\s*ms', text)
    ...
    parsed = {
        ...
        'cpu_wall_ms': float(cpu_wall.group(1)) if cpu_wall else 0.0,
...
def main(manifest_path, graphs_dir, out_dir):
    ...
    with open(summary_csv, 'w', newline='') as f:
        fieldnames = [
            ...
            'gpu_wall_ms', 'cpu_wall_ms', 'gpu_cpu_util_pct',
    ...
    for r in valid_rows:
        ...
        if r.get('cpu_wall_ms', 0) > 0:
            gpu_wall.setdefault('cpu_baseline', {})[value] = r['cpu_wall_ms']
